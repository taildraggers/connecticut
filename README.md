...
Daily aggregator of Connecticut-located taildragger classified listings,
...
[Colorado](https://github.com/taildraggers/colorado) repos' approach, with
the state filter swapped to "CT".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, CT` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Connecticut"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/connecticut/`
...
```html
<iframe
  src="https://taildraggers.github.io/connecticut/"
  title="Taildragger Ads in Connecticut"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_CONNECTICUT_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.

"""Aggregates Connecticut-located listings from every taildraggers.com
...
ones whose location ends in ", CT".
...
This repo is a straight copy of the companion Alabama/Alaska/Arizona/Arkansas/California/Colorado repos'
approach, with the state filter swapped to "CT".
"""
...
PAGE_TITLE = "Taildragger Ads in Connecticut"
STATE_ABBR = "CT"
...
_CONNECTICUT_LOCATION_RE = re.compile(r",\s*CT\s*$", re.IGNORECASE)


def _is_connecticut(location: str) -> bool:
    return bool(_CONNECTICUT_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        connecticut_rows = [row for row in rows if _is_connecticut(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(connecticut_rows)} in Connecticut")
        all_listings.extend(connecticut_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Connecticut listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Connecticut listings: {len(listings)}")
